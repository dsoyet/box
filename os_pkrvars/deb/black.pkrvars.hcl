...
boot_command            = ["<wait>e<down><down><down><leftCtrlOn>k<leftCtrlOff>    linux    /install.amd/vmlinuz net.ifnames=0 console-setup/ask_detect=false auto console-setup/layoutcode=us console-setup/modelcode=pc105 debconf/frontend=noninteractive debian-installer=en_US fb=false preseed/url=http://{{ .HTTPIP }}:{{ .HTTPPort }}/common/black.cfg auto=true priority=critical --- quiet<leftCtrlOn>x<leftCtrlOff>"]
